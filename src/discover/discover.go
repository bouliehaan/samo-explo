...
//
// samo-server has answered a UDP probe with its own address for as long as the
// Android and desktop clients have existed — it is how they find a server
// without being told one. Using it here means the setup wizard opens with the
// server address already in the box, instead of asking for something the
// network can answer by itself.
//
...
// samo-server itself run with host networking — and why the shipped compose
// file runs this with host networking too.
package discover
