...
if [ "$SKIP_GATES" -eq 0 ]; then
  # The frontend is go:embed'ed, so a Go build fails outright without it.
  echo "==> Building the web UI"
  ( cd src/web/frontend && npm ci --silent && npm run build )

  echo "==> go build"
  go build ./...

  # golangci-lint is what CI runs, and it catches things vet does not
  # (errcheck above all). Pinned to the version in .github/workflows.
  if command -v docker >/dev/null 2>&1; then
    echo "==> golangci-lint"
    docker run --rm -v "$PWD":/app -w /app golangci/golangci-lint:v2.13.2 golangci-lint run
  else
    echo "==> golangci-lint SKIPPED (no docker)"
  fi
else
